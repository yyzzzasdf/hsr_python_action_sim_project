...
Python 版行动轴仿真（对应原 MATLAB 版本），用于在固定规则下模拟三角色在「行动值（AV）」时间轴上的出手顺序、花火拉条效果，并评估**拉条浪费**与 **Archer / L 是否严格交替**。
...
- **ActualAltOK**：Archer / L 在 `action_df` 中的实际行动序列是否 A-L-A-L…
...
| 策略 | 行为 |
|------|------|
| `alternate`（默认） | 按花火行动次数奇偶：奇→Archer，偶→L |
| `always_archer` / `always_l` | 固定拉一人 |
...
1. 侧边栏：总 AV、三人速度、花火 / L 规则、拉条策略、竖线错开
2. 调用 `simulate_action()` → 顶部 metrics + 交替提示
3. **Tab1** 合并行动轴 | **Tab2** 分角色轨 | **Tab3** 表格 + Excel 下载
4. **Tab4** 固定花火速度，网格扫描 Archer×L → 表格 + 浪费/交替热力图 + Excel
...
花火=160，Archer 100–140 步长 5，L 90–130 步长 5 → `speed_sweep_result.xlsx` + 两张热力图 HTML
...
| 扫描 `result_df` | vArcher, vL, WasteTotalPercent, ActualAltOK, … | 热力图与鲁棒性分析 |
