...
c3.metric("L行动次数", int((action_df["Character"] == "L").sum()))
c4.metric("花火拉条总浪费", f"{waste_total:.2f}%")

c5, c6 = st.columns(2)
c5.info("Archer / L实际行动：" + ("严格交替" if flags["ActualAltOK"] else "不是严格交替"))
...
with tab4:
    ...
    st.caption("默认固定当前花火速度，扫描 Archer 和 L 速度。")
    ...
    if run_sweep:
        ...
        total = len(v_archer_list) * len(v_l_list)
        count = 0
        ...
        for va in v_archer_list:
            for vl in v_l_list:
                sweep_params = SimParams(
                    tmax=float(tmax),
                    speed0=(int(sweep_v_sparkle), int(va), int(vl)),
                    sparkle_initial_advance=sparkle_initial_advance,
                    sparkle_advance=sparkle_advance,
                    l_speed_bonus=l_speed_bonus,
                    sparkle_policy=sparkle_policy,
                ...
                rows.append(
                    {
                        ...
                        "LTurns": int((a_df["Character"] == "L").sum()),
        ...
        fig_waste = plot_waste_heatmap(result_df, int(sweep_v_sparkle), v_archer_list, v_l_list)
        st.plotly_chart(fig_waste, use_container_width=True)

        fig_alt = plot_alternation_heatmap(result_df, int(sweep_v_sparkle), v_archer_list, v_l_list)
